...
public class DecisionTree {
	...
	public int buildTree(String measure, List<Integer> indexFeatures)
	{
		countTotal++;
		//System.out.println("build tree node:"+countTotal);
		//System.out.println("training size="+training.data.size());
		if (measure=="random" && training.data.size()==0)
		{
			isLeaf=true;
			children=null;
			cat=0;
		...
		for (int i=0;i<indexFeatures.size();i++)
			if (indexFeatures.get(i)==0) numUsed++;
		
		if (measure=="random" & (numUsed==indexFeatures.size()-1 || e<0.3))		
		{
			int numPos=0;
			int numNeg=0;
			for (Sample s : training.data)
		...
		if (measure=="random") 
		{
			rule=(int)(Math.random()*numValues);
			while( indexFeatures.get(rule)==0 || training.types.get(rule).intValue()==3 )
			{
		...
		for (int i=0;i<newData.size();i++)
		{
			...
			children.add(t);		
			ArrayList<Integer> indexFeatures2=new ArrayList<Integer>();
			if (measure=="random")
			{
				for (int j=0;j<numValues;j++)
				{
					indexFeatures2.add(new Integer(indexFeatures.get(j)));				
				}
				indexFeatures2.set(rule, 0);
			}
			t.buildTree(measure, indexFeatures2);
		}
		return 0;
	}
	...
	public int classifySample(Sample instance)
	{
		...
		DecisionTree dt=children.get(branch);
		return dt.classifySample(instance);
	}
	...
	public static void main(String[] args) {
		...
		*/
		...
		System.out.println("*************Now Building info_gain tree***********");
		System.out.println("Before pruning, the learned tree(pre-order traverse) based on info_gain is: ");
		countInter=0;
		countTotal=0;
		root.traverse(0);
		System.out.println("\nomitted nodes below......\n");
		...
		System.out.println("*************Now doing info_gain pre-pruning testing***********");
		double accuracy=root.classifySet(test);
		System.out.println("Accuracy: "+accuracy);		


		//make stochastic pruning with tuning set
		System.out.println("*************Now doing info_gain pruning***********");
		...
		System.out.println("*************Now doing info_gain post_pruning testing***********");
		accuracy=root.classifySet(test);
		System.out.println("Accuracy: "+accuracy);		
		...
		DecisionTree.scoreOfBestTreeFound=0.0;
		DecisionTree.selectedFeatures=null;
		DecisionTree root2=new DecisionTree(training);
		DecisionTree.thresholdMap=new TreeMap<Integer, Double>();
		root2.conToDis();
	
		//build tree, random splitting
		
		//divide the training set into 80% and 20%
		Dataset tuning2=root2.eightyTwenty();
		indexFeatures=new ArrayList<Integer>();
		for (int i=0;i<training.types.size();i++)
		{
			
			indexFeatures.add(1);
		}
		System.out.println("\n\n*************Now Building random tree***********");
		DecisionTree.countTotal=0;
		DecisionTree.countInter=0;
		root2.buildTree("random", indexFeatures);
		System.out.println("Number of internal nodes is: "+DecisionTree.countInter);
		System.out.println("Total nodes: "+DecisionTree.countTotal);
		System.out.println();
		
		featureSubset=new TreeSet<String>();
		System.out.println("Before pruning, the learned tree(pre-order traverse) based on random is: ");
		countInter=0;
		countTotal=0;
		root2.traverse(0);
		System.out.println("\nomitted nodes below......\n");
		...
		System.out.println("*************Now doing random pre-pruning testing***********");
		accuracy=root2.classifySet(test);
		System.out.println("Accuracy: "+accuracy);		


		//make stochastic pruning with tuning set
		System.out.println("*************Now doing random pruning***********");
		DecisionTree tree2=root2.pruning(5,tuning2);
		root2=tree2;
		System.out.println();
		countInter=0;
		countTotal=0;
		...
		//make classification on test set
		System.out.println("*************Now doing random post-pruning testing***********");
		accuracy=root2.classifySet(test);
		System.out.println("Accuracy: "+accuracy);		
	}
}
